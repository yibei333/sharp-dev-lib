...
public static class FileUtil
{
    ...
    {
        if (filePath.IsNull()) return string.Empty;
    ...
    {
        if (filePath.IsNull()) throw new ArgumentNullException("path can not be null");
        if (bytes.IsEmpty()) throw new InvalidOperationException($"unable to save empty bytes to file");
    ...
    {
        if (filePath.IsNull()) throw new ArgumentNullException("path can not be null");
        if (stream.IsNull()) throw new InvalidOperationException($"unable to save empty stream to file");
        if (File.Exists(filePath)) File.Delete(filePath);
    ...
    {
        if (directory.IsNull()) throw new ArgumentNullException($"directory('{directory}') can't be empty");
