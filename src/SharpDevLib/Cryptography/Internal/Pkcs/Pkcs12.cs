...
namespace SharpDevLib.Cryptography.Internal.Pkcs;
...
internal static class Pkcs12
{
    ...
    static byte[] GetCertificateBag(X509Certificate2 certificate, string password, byte[] localKeyId)
    {
        ...
        using var hMAC = new HMACSHA256(password.Utf8Decode());
    ...
    static byte[] GetPrivateKeyBag(X509Certificate2 certificate, string privateKey, string password, byte[] localKeyId)
    {
        ...
        var encryptedPrivateKey = Pkcs8.EncodeEncryptedPrivateKey(pkcs8PrivateKey, password.Utf8Decode());
    ...
    static byte[] GetMacDerivedKey(byte[] saltOrigin, byte[] passwordOrigin, int iterationCount)
    {
        ...
        salt = [.. saltList.Take(salt.Length)];
        ...
        password = [.. passwordList.Take(password.Length)];
        ...
        {
            //1.A
            var A = algorithm.ComputeHash([.. D, .. I]);
            for (int j = 1; j < r; j++)
            {
    ...
    static byte[] GetBigEndianFormatPassword(byte[] passwordOrigin)
    {
        ...
        bigEndianFormatPasswordList.AddRange([0, 0]);
        return [.. bigEndianFormatPasswordList];
    }
}
