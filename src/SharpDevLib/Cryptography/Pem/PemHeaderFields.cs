...
internal class PemHeaderFields
{
    public PemHeaderFields(string? procType, string? dEKInfo)
    {
        ...
        {
            var array = DEKInfo?.Split([':'], StringSplitOptions.RemoveEmptyEntries).Last().Trim().Split([','], StringSplitOptions.RemoveEmptyEntries);
            if (array is null || array.Length != 2) throw new Exception("DEK-INFO require 2 fields");
            DEKInfoAlgorithm = array[0];
            DEKInfoAlgorithmFileds = array[0].Split(['-'], StringSplitOptions.RemoveEmptyEntries);
            DEKInfoIV = array[1];
