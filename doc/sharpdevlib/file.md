## 1. ```GetFileExtension(string filePath, bool includePoint = true)```
根据文件名或路径获取文件扩展名

``` csharp
//示例
Console.WriteLine("foo".GetFileExtension());//""
Console.WriteLine("foo.txt".GetFileExtension());//".txt"
Console.WriteLine("foo.txt".GetFileExtension(false));//"txt"
```

## 2. ```GetFileName(string filePath, bool includeExtension = true)```
根据文件名或路径获取文件名

``` csharp
//示例
Console.WriteLine("foo".GetFileName());//"foo"
Console.WriteLine("foo.txt".GetFileName());//"foo.txt"
Console.WriteLine("foo.txt".GetFileName(false));//"foo"
Console.WriteLine("foo/bar.txt".GetFileName());//"bar.txt"
Console.WriteLine("foo/bar.txt".GetFileName(false));//"bar"
```

## 3. ```GetMimeType```
根据文件名或路径获取Mime类型

``` csharp
//示例
Console.WriteLine("foo".GetMimeType());//""
Console.WriteLine("foo.txt".GetMimeType());//"text/plain"
Console.WriteLine("foo/bar.txt".GetMimeType());//"text/plain"
```

## 4. ```EnsureDirectoryExist```
确保文件夹路径存在，如果不存在则创建

``` csharp
//示例
@"c:\test-direcory".EnsureDirectoryExist();
```

## 5. ```EnsureFileExist```
确保文件路径存在，如果不存在抛出异常

``` csharp
//示例
@"c:\test-direcory\foo.txt".EnsureFileExist();
```

## 6. ```ToFileSizeString```
文件字节大小(小于0引发异常)转换为字符串,最大到PB

``` csharp
//示例
Console.WriteLine(5L.ToFileSizeString());//5Byte
Console.WriteLine(2045L.ToFileSizeString());//2KB
Console.WriteLine(4062L.ToFileSizeString());//4.51KB
Console.WriteLine(4730880L.ToFileSizeString());//4.51MB
Console.WriteLine(4844421120L.ToFileSizeString());//4.51GB
Console.WriteLine(4960687226880L.ToFileSizeString());//4.51TB
Console.WriteLine(5079743720325120L.ToFileSizeString());//4.51PB
```

## 7. ```CombinePath(string leftPath, string rightPath)```
合并路径

``` csharp
//示例
Console.WriteLine("foo".CombinePath("bar"));//"foo/bar"
Console.WriteLine("foo/bar".CombinePath("baz"));//"foo/bar/baz"
Console.WriteLine(@"foo\bar".CombinePath("baz"));//"foo/bar/baz"
```

## 8. ```FormatPath```
格式化路径:去掉前后空格,统一将```\```改为```/```

``` csharp
//示例
Console.WriteLine(@" foo\bar ".FormatPath());//"foo/bar"
```

## 9. ```FileBase64Encode```
通过文件路径将文件转换为Base64字符串

``` csharp
//示例
var path=@"foo\bar.png";
var base64=path.FileBase64Encode();
//data:image/png;base64,....
```

## 10. ```FileBase64Decode```
将Base64文件字符串解析为字节数组

``` csharp
//示例
var base64=@"data:image/png;base64,";
var bytes=base64.FileBase64Encode();
//bytes为一张图片的字节数组,可以直接将其保存为文件
```

## 11. ```RemoveFileIfExist```
如果文件存在则删除

``` csharp
//示例
"foo/bar.txt".RemoveFileIfExist();
```

## 12. ```OpenOrCreate```
根据文件信息打开或创建文件流

``` csharp
using var fileStream = new FileInfo("foo/bar").OpenOrCreate();
```

## 13. ```CopyToAsync```
拷贝流,本不应该放在这里

``` csharp
//示例
public async void CopyStream()
{
    using var sourceStream=new FileInfo("foo.txt").OpenOrCreate();
    using var targetStream=new FileInfo("bar.txt").OpenOrCreate();
    await sourceStream.CopyToAsync(targetStream,CancellationToken.None,TransferedNotify);
}

//已传输字节长度通知
public void TransferedNotify(long transfered)
{
    Console.WriteLine(transfered.ToString());
}

```

## 14. ```SaveToFile```
保存文件

###### 14.1 ```SaveToFile(byte[] bytes, string filePath, bool throwIfFileExist = false)```
将字节数组保存在文件中
* 如果throwIfFileExist=false,将覆盖已存在的文件
* 如果throwIfFileExist=true且文件已存在将抛出异常

``` csharp
//示例
var bytes = "foo.bar".ToUtf8Bytes();
var path = AppDomain.CurrentDomain.BaseDirectory.ombinePath("foo.txt");
bytes.SaveToFile(path);
```

###### 14.2 ```SaveToFileAsync(byte[] bytes, string filePath, CancellationToken? cancellationToken, bool throwIfFileExist = false)```
同上的异步方法

###### 14.3 ```SaveToFile(Stream stream, string filePath, bool throwIfFileExist = false)```
将流保存在文件中
* 如果throwIfFileExist=false,将覆盖已存在的文件
* 如果throwIfFileExist=true且文件已存在将抛出异常

``` csharp
//示例
using var stream = new MemoryStream("foo.bar".ToUtf8Bytes());
var path = AppDomain.CurrentDomain.BaseDirectory.CombinePath("foo.txt");
stream.SaveToFile(path);
```

###### 14.4 ```SaveToFileAsync(Stream stream, string filePath, CancellationToken? cancellationToken, bool throwIfFileExist = false)```
同上的异步方法

